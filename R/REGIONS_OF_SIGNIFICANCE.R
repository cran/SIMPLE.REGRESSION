...
# specify namesIVMOD_raw. The J-N computations will be based on these variables being
# in these positions.

# If there are covariates in addition to IV & MOD as predictors, or if the order
# is not IV then MOD, then namesIVMOD_raw must be specified.

# The namesIVMOD_model argument can be used to id the key terms () from an lme model
...
# If model is an lme object & IV is a two-level factor, then namesIVMOD_model
...
if (is.null(JN.data)) {
	message('\n\nA SIMPLE.REGRESSION model was provided, but it does not contain data')
...
if (inherits(model, 'lme')) {   # if (class(model) == 'lme') {
	...
	# if namesIVMOD_raw = NULL, the IV & MOD names are taken from the lme model, from the 2nd & 3rd terms
	if (is.null(namesIVMOD_raw)) { 
		IV  <- noms[2]
		MOD <- noms[3]
	...
	# not always the same are the names in rawdata
...
#			if (length(unique((rawdata[,IV]))) == 2) {
	...
	# get B & S
	if (is.null(namesIVMOD_model)) {
		B <- fixef(model)              # B[1] = int,  B[2] = IV,  B[3] = MOD,  B[4] = product
		S <- stats::vcov(model)
	} else {
		B <- fixef(model)[c('(Intercept)',namesIVMOD_model)]
		S <- stats::vcov(model)[c('(Intercept)',namesIVMOD_model), c('(Intercept)',namesIVMOD_model)]
	}
...
class(ROSoutput) <- "SIMPLE.REGRESSION"
